...
export default function Table(intern) {
  ...
  const { paginatedInterns, totalPages } = useMemo(() => {
    ...
    return {
      paginatedInterns: paginatedData,
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-300">
                  ...
                  <th className="px-6 py-3 text-center text-sm font-medium bg-neutral-100 text-black">
                    No.
                  </th>
                  <th className="px-6 py-3 text-left text-sm font-medium bg-neutral-100 text-black">
                    Nama
                  </th>
                  <th className="px-6 py-3 text-center text-sm font-medium bg-neutral-100 text-black">
                    NIM
                  ...
                  {isAdmin && (
                    <th className="px-6 py-3 text-sm bg-neutral-100 text-black">
                      Aksi
                    </th>
                  )}
                </tr>
                ...
                {paginatedInterns.length > 0 ? (
                  paginatedInterns.map((intern, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        {formatDate(intern.tanggalMulai)} –{" "}
                        {formatDate(intern.tanggalSelesai)}
                      </td>
                      <td className="px-6 py-4 text-center">
                        <div
                          className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getDivisionColor(
                            intern.divisi
                          )}`}
                        >
                          {intern.divisi || "-"}
                      ...
                      <td className="px-6 py-4 text-center">
                        <div
                          className={`inline-flex items-center capitalize px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(
                            intern.status
                          )}`}
                        >
                          {intern.status}
                      ...
                      {isAdmin && (
                        <td className="px-6 py-4 text-center flex gap-2 justify-center">
                          ...
                          <button
                            onClick={() =>
                              openDeleteModal(intern._id, intern.nama)
                            }
